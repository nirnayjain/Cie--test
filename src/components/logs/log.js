...
class Press extends React.Component {
  ...
  componentDidMount() {
    axios.get(`logs/fetch`).then((res) => {
      const blogs = res.data.reverse();
      console.log(blogs);
      this.setState({ blogs, loading: true });
    });
    this.unsubscribe = axios.get(`logs/fetch`).then((res) => {
      const blogs = res.data.reverse();
      console.log(blogs);
      this.setState({ blogs, loading: true });
    });
      ...
      this.state.blogs.slice(offset, offset + PER_PAGE).map((blog, index) => {
        ...
        return (
          <tr key={index}>
            <td>{index + 1}</td>
            <td>
              <div className="limited-text">{blog.ip}</div>
            </td>
            <td> {new Date(blog.createdAt).toDateString()}</td>
            <td>
              <Link to={`/view_log/${blog._id}`}>
      ...
      <div>
        ...
        <div className="admin-wrapper col-12">
          <div className="admin-content">
            <div className="admin-head">Logs</div>
